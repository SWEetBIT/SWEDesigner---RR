	\section{Riunione 2}
	  ...
	  \subsection{Domande e Risposte}
	   \begin{itemize}
	   	   	...
	   	   	\item 
	   	%dDOMANDA
	   		\textbf{Per la gestione tra i \glossaryItem{diagrammi} delle \glossaryItem{classi} e dei \glossaryItem{metodi}, abbiamo pensato di mettere a disposizione dell'\glossaryItem{utente} una finestra principalmente divisa in 3 parti. La prima parte comprende gli strumenti necessari a disegnare i vari \glossaryItem{diagrammi}, la seconda finestra è quella principale e consiste nello schema delle \glossaryItem{classi} e, in caso di doppio click sul \glossaryItem{metodo} di una \glossaryItem{classe}, si aprirà un \glossaryItem{activity diagram}, col quale l'\glossaryItem{utente} potrà creare e definire il corpo del \glossaryItem{metodo}. Nella terza finestra, l'\glossaryItem{utente} ha a disposizione un \glossaryItem{activity diagram} in sola lettura, sul quale è possibile visualizzare il flusso dell'intero progetto. Come le sembra la nostra soluzione? } \\
	   	%RISPOSTA
	    	\justifying     		
Questa disposizione è apprezzata, l' importante è non saltare il \glossaryItem{diagramma} delle \glossaryItem{classi} e trattare il \glossaryItem{activity diagram} in modo da poter generare il \glossaryItem{codice}..\\
